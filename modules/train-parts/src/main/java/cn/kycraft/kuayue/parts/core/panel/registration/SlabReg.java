package cn.kycraft.kuayue.parts.core.panel.registration;

import cn.kycraft.kuayue.parts.core.panel.SlabBlock;
import cn.kycraft.kuayue.parts.core.panel.SlabBlockItem;
import lib.kasuga.registration.Reg;
import lib.kasuga.registration.minecraft.block.BlockReg;
import lib.kasuga.registration.minecraft.block.BlockRegConfigurations;
import lib.kasuga.registration.minecraft.item.ItemReg;
import lib.kasuga.registration.minecraft.item.ItemRegConfigurations;
import net.minecraft.world.flag.FeatureFlag;
import net.minecraft.world.level.block.Block;
import net.minecraft.world.level.block.state.BlockBehaviour;

import java.util.function.Function;

public class SlabReg<T extends SlabBlock> extends Reg<SlabReg<T>, Block> implements BlockRegConfigurations<SlabReg<T>>, ItemRegConfigurations<SlabReg<T>> {
    private final BlockReg<T> blockReg;
    private final ItemReg<SlabBlockItem> itemReg;

    public SlabReg(String name, Function<BlockBehaviour.Properties, T> blockSupplier) {
        this.blockReg = BlockReg.of(name, blockSupplier).setParent(this);
        this.itemReg = ItemReg.of(name, (i)->new SlabBlockItem(blockReg.getEntry(), i)).setParent(this);
    }

    @Override
    public SlabReg<T> requiredFeatures(FeatureFlag[] requiredFeatures) {
        return BlockRegConfigurations.super.requiredFeatures(requiredFeatures);
    }

    @Override
    public SlabBlock getEntry() {
        return null;
    }

    public ItemReg<SlabBlockItem> getItem() {
        return itemReg;
    }
}
